...
export function MistakesPage() {
  ...
  if (index >= questions.length) return <main className="practice-page"><section className="result-card"><MiniCelebration type="complete" /><span>🎯</span><h1>Тренировка закончена</h1><strong>{correct} из {questions.length}</strong><p>Возвращайся позже: список обновится после новых квизов.</p><Link href="/study" className="link-button">Вернуться к плану</Link></section></main>;
