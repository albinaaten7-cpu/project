...
export function LessonBrief({ lesson, quizLoading = false, onStart }: { lesson: DailyLesson; quizLoading?: boolean; onStart: () => void }) {
  return (
    <section className="lesson-brief">
      ...
      {lesson.sections.map((section) => <article className="lesson-section" key={`${section.subject}-${section.topic}`}><div className="quiz-meta"><span>{section.subject}</span><span>{section.topic}</span></div><p className="lesson-summary">{section.summary}</p><div className="lesson-grid"><div><b>Главное</b><ul>{section.keyPoints.map((point) => <li key={point}>{point}</li>)}</ul></div><div><b>Разбор примера</b><p>{section.workedExample}</p></div></div><ExtraMaterials subject={section.subject} topic={section.topic} /></article>)}
