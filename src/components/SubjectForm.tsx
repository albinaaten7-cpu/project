...
export function SubjectForm({ onAdd, settings }: Props) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    setBusy(true);
    try {
      await onAdd({ name: selectedName, current_grade: currentGrade, target_grade: targetGrade, exam_date: examDate, topics: topics.trim() });
      setSelectedName('');
      setExamDate('');
      setTopics('');
    } finally {
  ...
  return (
    <section className="panel">
      ...
      <form className="subject-form" onSubmit={submit}>
        ...
        <label className="wide">Дата экзамена<input type="date" value={examDate} min={new Date().toISOString().slice(0, 10)} onChange={(e) => setExamDate(e.target.value)} required /></label>
