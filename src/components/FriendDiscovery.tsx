...
type Props = {
  ...
  onAdd: (user: DiscoveredUser) => Promise<void>;
};

export function FriendDiscovery({ users, friendships, currentUserId, query, onQuery, onAdd }: Props) {
  ...
  return (
    <section className="friend-discovery">
      <div className="friends-heading"><div><span>{query ? 'Поиск' : 'Рядом сейчас'}</span><h2>{query ? 'Найти пользователя' : 'Кто сейчас в Треке?'}</h2></div><b>{visible.length}</b></div>
      ...
      {visible.length ? <div className="nearby-users">{visible.map((user) => {
        ...
        return <article key={user.userId}><div className="nearby-avatar"><span>{user.displayName.slice(0, 1).toUpperCase()}</span>{user.lastSeenAt && Date.parse(user.lastSeenAt) >= nearbyLimit && <i />}</div><div><strong>{user.displayName}</strong><small>@{user.nickname} · {user.xp} XP</small></div><div className="nearby-actions">{!friendship && <button onClick={() => void onAdd(user)}>+ Добавить в друзья</button>}{sent && <span>Запрос отправлен</span>}{isFriend && <span>✓ Уже в друзьях</span>}</div></article>;
      })}</div> : <div className="friends-empty"><span>⌁</span><h3>{query ? 'Никого не нашли' : 'Пока никого рядом'}</h3><p>{query ? 'Проверь имя или никнейм и попробуй ещё раз.' : 'Активные ученики появятся здесь, как устройства в AirDrop.'}</p></div>}
    </section>
  );
}
