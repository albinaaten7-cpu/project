...
export function QuizResult({ day, totalDays, score, totalQuestions, mistakes, registered, reviewMode, onReview, onNextDay }: Props) {
  if (reviewMode) return <section className="result-card"><MiniCelebration type="complete" /><span>✨</span><h1>Ошибки разобраны!</h1><p>Ты ещё раз прошёл сложные вопросы. Теперь материал закрепится лучше.</p><button onClick={onNextDay}>{day < totalDays ? `Перейти к дню ${day + 1} →` : 'Вернуться к уроку'}</button></section>;

  return (
    <section className="result-card">
      <MiniCelebration type="complete" />
